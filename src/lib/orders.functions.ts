...
export const placeOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: items, error: cartErr } = await supabase
      .from("cart_items")
      .select("id, quantity, size, remark, product:products(id, slug, name, sku, price, image_url, in_stock, gross_weight, net_weight)")
      .eq("user_id", userId);
    ...
    type CartRow = {
      ...
      product: { id: string; slug: string | null; name: string; sku: string | null; price: number | string; image_url: string | null; in_stock: boolean | null; gross_weight: number | string | null; net_weight: number | string | null } | null;
    };

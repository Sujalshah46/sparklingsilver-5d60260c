...
import { adjustStock, bulkApplyInventory, getAdminInventory } from "@/lib/inventory.functions";
...
function InventoryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-inventory", page, q, filter],
  ...
  const mut = useMutation({
    ...
    onMutate: async (vars) => {
      ...
      qc.setQueryData<any>(["admin-inventory", page, q, filter], (old: any) => {
        if (!old) return old;
        return {
          ...old,
          products: (old.products ?? []).map((p: any) => p.id === vars.product_id
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.prev) qc.setQueryData(["admin-inventory", page, q, filter], ctx.prev);
    ...
    <MobileShell title="Inventory">
      <div className="p-4 space-y-3">
        ...
        <div className="relative">
          ...
          <Input value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} placeholder="Search by name or SKU" className="pl-9" />
        </div>
        ...
        {data && data.pages > 1 && (
          <div className="flex items-center justify-between border-t border-border pt-4 mt-4">
            ...
            <span className="text-xs text-muted-foreground">
              Page {page} of {data.pages} (Total: {data.total})
            </span>
            <Button
              variant="outline"
              size="sm"

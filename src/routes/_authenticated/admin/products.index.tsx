...
import { deleteProduct, getAdminProducts } from "@/lib/products.functions";
...
function ProductsAdmin() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchProducts({ data: { page, search: q || undefined, limit: 20 } }),
  });

  const products = data?.products ?? [];
  const filtered = products; // already filtered at database layer

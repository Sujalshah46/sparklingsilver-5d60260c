  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("products")
        .select("id, name, sku, slug, price, image_url, stock_quantity, low_stock_threshold, is_new, is_bestseller, categories(name)")
                ...
                <li key={p.id} className="rounded-xl border border-border bg-card p-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 font-serif text-sm text-burgundy">{inr(p.price)}</p>

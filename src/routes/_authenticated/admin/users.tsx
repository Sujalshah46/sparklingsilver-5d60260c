...
import { UserPlus, Copy, KeyRound, Power, Send, RefreshCw, Mail, CheckCircle2, MessageCircle, ShieldCheck, Trash2, Search } from "lucide-react";
...
function AdminUsersPage() {
  ...
  const users = useQuery({
    ...
    queryFn: () => listUsers({ data: { page, search: q || undefined, limit: 20 } }),
  });
          ...
          <TabsContent value="users" className="space-y-2">
            <div className="relative mb-2">
              ...
              <Input
                ...
                placeholder="Search users by name, contact, email or phone..."
                className="pl-9"
              />
            </div>
            {users.isLoading && <p className="text-xs text-muted-foreground">Loading…</p>}
            {(users.data?.users ?? []).map((u: UserRow) => (
            ...
            {users.data && users.data.pages > 1 && (
              <div className="flex items-center justify-between border-t border-border pt-4 mt-4">
                ...
                <span className="text-xs text-muted-foreground">
                  Page {page} of {users.data.pages} (Total: {users.data.total})
                </span>
                <Button
                  variant="outline"
                  size="sm"

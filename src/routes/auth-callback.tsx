...
function AuthCallbackPage() {
  ...
  useEffect(() => {
    // Wait until the Supabase client has finished exchanging the OAuth code
    // from this URL (detectSessionInUrl) and confirmed a real session before
    // navigating away — otherwise the root auth gate sees no session and
    // bounces the user back to /auth.
    ...
    // getSession() resolves only after the client's initial URL session
    // detection/exchange completes, so this covers the case where SIGNED_IN
